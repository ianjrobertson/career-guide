 ...
 */
export function saveDraft(problemId: string, answer: string, thoughts: string): void {
  ...
  localStorage.setItem(`${DRAFT_STORAGE_KEY}${problemId}`, JSON.stringify(draft));
}

/**
 * Load draft answer from localStorage
 */
export function loadDraft(problemId: string): { answer: string; thoughts: string } | null {
