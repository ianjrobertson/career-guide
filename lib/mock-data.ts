...
export const MOCK_MAJORS: Major[] = [
  {
    ...
    description:
        'Design and build software systems, work with algorithms and data structures',
    category: 'Technology'
  },
  {
    id: 'major-2',
    ...
    description:
        'Learn to manage organizations, analyze markets, and make strategic decisions',
    category: 'Business'
  },
  {
    id: 'major-3',
    name: 'Psychology',
    description:
        'Study human behavior, mental processes, and therapeutic techniques',
  ...
  },
  {
    ...
    description:
        'Analyze texts, develop critical thinking, and master written communication',
    category: 'Humanities'
  },
  {
    id: 'major-6',
...
export function getUserAttempts(userId: string = 'mock-user-1'):
    UserProblemAttempt[] {
  return MOCK_USER_ATTEMPTS.filter(a => a.user_id === userId);
}

export function getUserRecommendations(userId: string = 'mock-user-1'):
    MajorRecommendation[] {
...
export function getMajorsBySkillId(skillId: string): Major[] {
  ...
  return skill.major_ids.map(majorId => getMajorById(majorId))
...
export function generateMockFeedback(
    problemId: string, userAnswer: string, skillId: string): ProblemFeedback {
  const skill = getSkillById(skillId);
  ...
  if (answerLength > 200) {
    feedbackText =
        `Great work! You provided a thorough and well-thought-out response. Your answer demonstrates strong understanding of ${
            skill?.name || 'this skill'}.`;
    strengths =
        ['Detailed explanation', 'Clear reasoning', 'Good use of examples'];
    improvements =
        ['Consider organizing your thoughts with bullet points for clarity'];
  } else if (answerLength > 100) {
    feedbackText =
        `Good effort! You covered the key points and showed solid thinking around ${
            skill?.name || 'this skill'}. Your answer is on the right track.`;
    strengths = ['Addressed the main question', 'Logical approach'];
    improvements = [
      'Try adding more specific examples',
      'Consider exploring alternative perspectives'
    ];
  } else if (answerLength > 30) {
    feedbackText = `Nice start! You've identified some important points about ${
        skill?.name ||
        'this skill'}. With a bit more depth, this could be even stronger.`;
    strengths = ['Concise response', 'Got the basics right'];
    improvements = [
      'Expand on your reasoning', 'Add more details to support your answer',
      'Consider what-if scenarios'
    ];
  } else {
    feedbackText = `Thanks for giving it a try! ${
        skill?.name ||
        'This skill'} can be challenging. Don't worry about getting it perfect - the goal is to explore and learn what you enjoy.`;
    strengths =
        ['You engaged with the problem', 'Every attempt is valuable learning'];
    improvements = [
